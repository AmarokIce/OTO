...
class Rfill : CommandBase() {
    override fun getCommandName(): String {
        return "rfill"
    }

    override fun getCommandUsage(sender: ICommandSender): String {
        return "/rfill [start x] [start y] [start z] [end x] [end y] [end z] [Block RegisterName or Block Id] [meta] [random]"
    }
    ...
    override fun processCommand(sender: ICommandSender, list: Array<String>) {
        ...
        val randomInt: Int = if (Integer.parseInt(list[8]) == 0) 2 else Integer.parseInt(list[8])
        ...
        if (sx < ex && sy < ey && sz < ez) {
            sender.addChatMessage(ChatComponentText("Worry! Cannot make start bigger than end ! "))
            return
        }

        val block: Block = getBlockByText(sender, list[6]) ?: return
        val meta: Int = Integer.parseInt(list[7])
        var i: Int  // A sandman.
        ...
        for (posX: Int in sx .. ex)
            for (posY: Int in sy .. ey)
                for (posZ: Int in sz..ez)
                    if (Random().nextInt(randomInt) != 0) player.worldObj.setBlock(posX, posY, posZ, block, meta, 3)
                    else continue
    }
